...
public class MainWindow1 extends JFrame { //метод в котором используем фрэмворк СВИНГ, в данном случае  джйфрэм по созданию поля
    ...
    MainWindow1() { // задаем окошко
        prepareAppWindow(); // метод как сделать окошко
        prepareGameSettings(); // метод по расположению кнопок
        prepareButtons(); // метод начала игры
        prepareSettingsControls();
        prepareGameLog();

        gameMap = new GameMap1(this);
...
//                putLog(" User choose " + sliderMapSizeSetup.getValue());
    ...
    private void prepareSettingsControls() {
        ...
        panelControl.add(labelMapSize); // добавляем надпись по полю
        panelControl.add(sliderMapSizeSetup); // добавляем ползунок выбора размера
        panelControl.add(labelWinLength); // добавляем надпись по выигрышу
        panelControl.add(sliderWinLengthSetup); // добавляем ползунок и условиямит выигрыша
        panelControl.add(btnStart); // добавляем кнопку начало
        panelControl.add(btnExit); // добавляем кнопку выход
        panelControl.add(btnClearLog); // добавляем кнопку стереть логи
    }
    ...
    void putLog(String msg) { // метод  по созданию вывода текста в логи
        gameLog.append(msg + " \n");
    }

    private void collectGameSetupAndLaunch() { // метод который будет выводить в логи размер окна например 3х3 5х5
        int mapSize = sliderMapSizeSetup.getValue(); // ну естественно водя по бегунку
        int winLen = sliderWinLengthSetup.getValue(); // и победные настройки
        ++round;
        putLog("--- Round" + round + " ---"); // будет выводить в логи очередность игр

        putLog("Map size is " + mapSize + "x" + mapSize + ". Win length is " + winLen); // будет выводить в логи каую карту и выбрал с победными данными
